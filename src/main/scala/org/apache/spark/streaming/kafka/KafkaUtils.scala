...
import org.apache.spark.streaming.kafka.mt.MTDirectKafkaInputDStream
...
object KafkaUtils {
  ...
  def createDirectStream[
    ...
    new MTDirectKafkaInputDStream[K, V, KD, VD, R](
      ssc, kafkaParams, fromOffsets, cleanedHandler, zk_offset_record_dir)
  }
  ...
  def createDirectStream[
    ...
    new MTDirectKafkaInputDStream[K, V, KD, VD, (K, V)](
      ssc, kafkaParams, fromOffsets, messageHandler, zk_offset_record_dir)
  }
...
private[kafka] class KafkaUtilsPythonHelper {
  ...
  def createDirectStreamWithoutMessageHandler(
      ...
      topics: JSet[String],
      fromOffsets: JMap[TopicAndPartition, JLong], 
      zk_offset_record_dir: String): JavaDStream[(Array[Byte], Array[Byte])] = {
    val messageHandler =
      (mmd: MessageAndMetadata[Array[Byte], Array[Byte]]) => (mmd.key, mmd.message)
    new JavaDStream(createDirectStream(jssc, kafkaParams, topics, fromOffsets, messageHandler, zk_offset_record_dir))
  }
  ...
  def createDirectStreamWithMessageHandler(
      ...
      topics: JSet[String],
      fromOffsets: JMap[TopicAndPartition, JLong], 
      zk_offset_record_dir: String): JavaDStream[Array[Byte]] = {
    val messageHandler = (mmd: MessageAndMetadata[Array[Byte], Array[Byte]]) =>
      new PythonMessageAndMetadata(mmd.topic, mmd.partition, mmd.offset, mmd.key(), mmd.message())
    val stream = createDirectStream(jssc, kafkaParams, topics, fromOffsets, messageHandler, zk_offset_record_dir).
      mapPartitions(picklerIterator)
  ...
  private def createDirectStream[V: ClassTag](
      ...
      zk_offset_record_dir: String): DStream[V] = {
